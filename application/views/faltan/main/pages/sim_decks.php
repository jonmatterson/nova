...
<?php if (isset($specs)): ?>
	<?php if (count($specs) > 0): ?>
		<?php foreach ($specs as $key => $value): ?>
			<?php echo '<h3>'.anchor('sim/decks/'.$key, $value['name'], array('class' => 'bold fontMedium')).'</h3>'.text_output($value['desc'].' '.anchor('sim/specs/'. $item['id'], 'View decks...', array('class' => 'bold')));?>
		<?php endforeach;?>
	<?php else: ?>
